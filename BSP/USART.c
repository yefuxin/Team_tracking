...
#elif USART_FLAG == 3
static USART_TypeDef* Using_USART = USART3;
#elif USART_FLAG == 4
static USART_TypeDef* Using_USART = USART4;
#elif USART_FLAG == 6
static USART_TypeDef* Using_USART = USART6;
#elif USART_FLAG == 7
static USART_TypeDef* Using_USART = UART7;
#elif USART_FLAG == 8
static USART_TypeDef* Using_USART = UART8;
#endif
...
#if Device_BoardType != Device_BoardType_2017
...
void USART7_RXDMA_Config(uint32_t USART7_DMABuff_addr, uint32_t buffsize)
{
	...
	DMA_DeInit(UART7_RX_DMA_STREAM);// 复位初始化DMA数据流 

	while (DMA_GetCmdStatus(UART7_RX_DMA_STREAM) != DISABLE);//确保DMA数据流复位完成

	DMA_InitStructure.DMA_Channel = UART7_RX_DMA_CHANNEL; //usart6 rx对应dma1，通道5，数据流1 	
	...
	DMA_Init(UART7_RX_DMA_STREAM, &DMA_InitStructure);//配置DMA2的数据流		 

	DMA_Cmd(UART7_RX_DMA_STREAM, ENABLE);//使能DMA

	while (DMA_GetCmdStatus(UART7_RX_DMA_STREAM) != ENABLE);// 等待DMA数据流有效
...
void USART8_RXDMA_Config(uint32_t USART8_DMABuff_addr, uint32_t buffsize)
{
	...
	DMA_DeInit(UART8_RX_DMA_STREAM);// 复位初始化DMA数据流 

	while (DMA_GetCmdStatus(UART8_RX_DMA_STREAM) != DISABLE);//确保DMA数据流复位完成

	DMA_InitStructure.DMA_Channel = UART8_RX_DMA_CHANNEL; //usart8 rx对应dma1，通道5，数据流6 	
...
//	DMA_InitStructure.DMA_Mode =  DMA_Mode_Normal;
	...
	DMA_InitStructure.DMA_MemoryBurst = DMA_MemoryBurst_Single; 	//存储器突发传输 16 个节拍   	
	DMA_InitStructure.DMA_PeripheralBurst = DMA_PeripheralBurst_Single;   //外设突发传输 1 个节拍 
	DMA_Init(UART8_RX_DMA_STREAM, &DMA_InitStructure);//配置DMA1的数据流		 

	DMA_Cmd(UART8_RX_DMA_STREAM, ENABLE);//使能DMA

	while (DMA_GetCmdStatus(UART8_RX_DMA_STREAM) != ENABLE);// 等待DMA数据流有效
